  ...
  return (
    <div style={styles.page}>
      ...
      <main style={styles.main}>
        ...
        {/* Charts Row 1: Health + Risk donut */}
        <div style={styles.row2}>
          <div style={styles.card}>
            <div style={styles.cardTitle}>Plant Health Distribution</div>
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={healthDist} dataKey="count" nameKey="status" cx="50%" cy="50%" outerRadius={80} label={({ status, percent }) => `${status} ${(percent*100).toFixed(0)}%`}>
                  {healthDist.map((e, i) => <Cell key={i} fill={HEALTH_COLORS[e.status] || '#4a6b4e'} />)}
                </Pie>
                <Tooltip formatter={(v, n) => [v, n]} contentStyle={{ background: '#161f18', border: '1px solid #243d28', borderRadius: '8px', fontFamily: "'DM Mono'" }} />
              </PieChart>
            </ResponsiveContainer>
          </div>
          <div style={styles.card}>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={riskDist} dataKey="count" nameKey="level" cx="50%" cy="50%" outerRadius={80} label={({ level, percent }) => `${level} ${(percent*100).toFixed(0)}%`}>
...
const styles = {
  ...
  sidebar: {
    ...
    background: '#161f18', borderRight: '1px solid #243d28',
