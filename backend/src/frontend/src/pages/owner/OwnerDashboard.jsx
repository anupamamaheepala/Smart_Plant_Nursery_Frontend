          ...
          return (
            <div key={m.model} style={aiStyles.modelCard}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                <div style={{ fontSize: '10px', color: '#4a6b4e', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{m.type}</div>
                <span style={{ fontSize: '10px', padding: '2px 8px', borderRadius: '9999px', fontWeight: '600', color: mc, background: mc + '15', border: `1px solid ${mc}44` }}>{m.member}</span>
              </div>
      ...
      <div style={aiStyles.chartRow}>
        {data.plant_health_importance && (
          <div style={aiStyles.chartCard}>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={data.plant_health_importance} layout="vertical" margin={{ top: 0, right: 20, left: 10, bottom: 0 }}>
                <XAxis type="number" tick={{ fill: '#4a6b4e', fontSize: 10, fontFamily: "'DM Mono'" }} tickLine={false} axisLine={false} unit="%" />
                <YAxis type="category" dataKey="feature" width={110} tick={{ fill: '#86a98a', fontSize: 11, fontFamily: "'DM Mono'" }} tickLine={false} axisLine={false} />
                <Tooltip formatter={(v) => [`${v}%`, 'Importance']} contentStyle={{ background: '#161f18', border: '1px solid #243d28', borderRadius: '8px', fontFamily: "'DM Mono'", fontSize: '12px' }} />
        ...
        {data.watering_importance && (
          <div style={aiStyles.chartCard}>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={data.watering_importance} layout="vertical" margin={{ top: 0, right: 20, left: 10, bottom: 0 }}>
                <XAxis type="number" tick={{ fill: '#4a6b4e', fontSize: 10, fontFamily: "'DM Mono'" }} tickLine={false} axisLine={false} unit="%" />
                <YAxis type="category" dataKey="feature" width={110} tick={{ fill: '#86a98a', fontSize: 11, fontFamily: "'DM Mono'" }} tickLine={false} axisLine={false} />
                <Tooltip formatter={(v) => [`${v}%`, 'Importance']} contentStyle={{ background: '#161f18', border: '1px solid #243d28', borderRadius: '8px', fontFamily: "'DM Mono'", fontSize: '12px' }} />
                <Bar dataKey="importance" radius={[0, 4, 4, 0]}>
...
export default function OwnerDashboard() {
  ...
  useEffect(() => {
    api.get('/predict/owner/insights').then(r => setAiInsights(r.data)).catch(console.error)
  }, [])
        ...
        <div style={styles.row2}>
          <div style={styles.card}>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={healthDist} dataKey="count" nameKey="status" cx="50%" cy="50%" outerRadius={80}
          ...
          <div style={styles.card}>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={riskDist} dataKey="count" nameKey="level" cx="50%" cy="50%" outerRadius={80}
          ...
          <div style={styles.card}>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={riskTrend} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
                ...
                <XAxis dataKey="time" tick={{ fill: '#4a6b4e', fontSize: 9, fontFamily: "'DM Mono'" }} tickLine={false} axisLine={false} interval="preserveStartEnd" />
                <YAxis tick={{ fill: '#4a6b4e', fontSize: 9 }} tickLine={false} axisLine={false} />
                <Tooltip content={<TIP />} />
                <Line type="monotone" dataKey="avg_risk" name="Risk Score" stroke="#fbbf24" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div style={styles.card}>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={envTrend} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#1a2e1d" />
                <XAxis dataKey="time" tick={{ fill: '#4a6b4e', fontSize: 9 }} tickLine={false} axisLine={false} interval="preserveStartEnd" />
                <YAxis tick={{ fill: '#4a6b4e', fontSize: 9 }} tickLine={false} axisLine={false} />
                <Tooltip content={<TIP />} />
          ...
          <div style={styles.card}>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={soilTrend} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
                ...
                <YAxis tick={{ fill: '#4a6b4e', fontSize: 9 }} tickLine={false} axisLine={false} />
                <Tooltip content={<TIP />} />
                <Line type="monotone" dataKey="avg_moisture" name="Soil Moisture %" stroke="#4ade80" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
          <div style={styles.card}>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={waterTrend} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
                ...
                <YAxis tick={{ fill: '#4a6b4e', fontSize: 9 }} tickLine={false} axisLine={false} />
                <Tooltip content={<TIP />} />
                <Line type="monotone" dataKey="avg_water" name="Water Level %" stroke="#22d3ee" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
            ...
            : (
              <div style={{ overflowX: 'auto' }}>
                <table style={styles.table}>
                  <thead>
                    <tr>{['Timestamp','Risk Score','Soil %','Temp °C','Humidity %','Water %','Root Status','Alert'].map(h => (
                  ...
                  <tbody>
                    {critEvents.map((e, i) => (
                      <tr key={i} style={{ borderBottom: '1px solid #1a2e1d' }}>
                        ...
                        <td style={{ ...styles.td, fontFamily: "'DM Mono', monospace" }}>{e.soil_moisture?.toFixed(1)}</td>
                        <td style={{ ...styles.td, fontFamily: "'DM Mono', monospace" }}>{e.air_temp?.toFixed(1)}</td>
                        <td style={{ ...styles.td, fontFamily: "'DM Mono', monospace" }}>{e.air_humidity?.toFixed(1)}</td>
                        <td style={{ ...styles.td, fontFamily: "'DM Mono', monospace" }}>{e.water_level_percent?.toFixed(1)}</td>
                        <td style={styles.td}>{e.Root_Water_status}</td>
                        <td style={{ ...styles.td, fontSize: '11px', color: e.alert === 'NORMAL' ? '#4ade80' : '#fbbf24' }}>{e.alert}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
